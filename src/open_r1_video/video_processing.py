...
def extract_k_frames_decord_cpu(video_path, image_factor: int = IMAGE_FACTOR, k=None, min_frames=None, save_image=False) -> torch.Tensor | list[Image.Image]:
    ...
    if duration_seconds <= 100:
        ...
        # Calculate additional frames for time beyond 100 seconds
        excess_time = duration_seconds - 100
        additional_100s_blocks = int(excess_time // 100)  # Complete 100-second blocks
        
        # Double frames for each additional 100-second block
        # Block 1 (100-200s): 8 additional frames
        # Block 2 (200-300s): 16 additional frames  
        # Block 3 (300-400s): 32 additional frames, etc.
        additional_frames = 0
        for block in range(additional_100s_blocks):
    ...
    k = min(k, 128)  # Cap k at 64 frames to avoid excessive memory usage
    # print(f"Total frames in video: {duration_seconds}, Extracting k={k} frames")
    indices = np.linspace(0, total_frames - 1, k, dtype=int)
    ...
    # if save_image:
    # first_frame = video[0]  # This is already a PIL Image
    # first_frame.save("test.png")
    # print(f"Saved first frame as test.png - size: {first_frame.size}")
