...
def process_questions(dataset: List[dict], output_path: str, surprise_scoring=False, scores_data=[], model_path=None) -> None:
    ...
    print("Surprise scoring:", surprise_scoring, "Scores data length:", len(scores_data))
    ...
    for item in tqdm(dataset):
        ...
        try:
            ...
            options = ""
            answer = extract_letter(item["answer"].replace("A.", "(A)").replace("B.", "(B)").replace("C.", "(C)").replace("D.", "(D)"))
            question = item["question"].replace("A.", "(A)").replace("B.", "(B)").replace("C.", "(C)").replace("D.", "(D)") 
            options   = []
            model_answer = video_eval_nframes(model, processor, question, options, preevent_frames, event_frames, postevent_frames)
