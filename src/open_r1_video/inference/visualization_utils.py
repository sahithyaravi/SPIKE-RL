...
def build_expl_panel(
    ...
    **black** for readability.
    ...
    for text, d_js in parsed:
        ...
        colour = (255, 253, 208) #tuple(int(c * 255) for c in cmap(frac)[:3])
...
def make_scrollbar_video_non_highlighted(
    ...
    *,
    ...
    for expl in explanations:
        p, h = build_expl_panel(expl, TARGET_W, font_big, memory=memory or "")
    ...
    for idx, (raw, s, panel) in enumerate(zip(frames, scores, padded_panels)):
        ...
        # spark‑line
        db.line(spark_pts[: idx + 1], fill=(30, 144, 255), width=3)  # Blue line

        # Current point highlight
        x_cur, y_cur = spark_pts[idx]
        db.ellipse([x_cur - 5, y_cur - 5, x_cur + 5, y_cur + 5], fill=(255, 215, 0))  # Gold dot

        # Cursor line
        db.line([(x_cur, 0), (x_cur, BAR_H)], fill=(200, 200, 200), width=2)
        ...
        text_x = x_cur - tw // 2
        text_y = (BAR_H - th) // 2
        db.rounded_rectangle([(text_x - 6, text_y - 4), (text_x + tw + 6, text_y + th + 4)], 
                            radius=6, fill=(255, 255, 255), outline=(30, 144, 255), width=2)
        db.text((text_x, text_y), txt, font=font_big, fill=(30, 144, 255))
        # stack
...
def make_scrollbar_video(
    ...
    *,
    out: str = "surprise_timeline.mp4",
    amusing_indices: Sequence[int] | None = None,
    surprised_indices: Sequence[int] | None = None,
    memory: str | None = None,
):
    """Compose MP4 with raw frame, *spark‑line only*, and coloured‑background panel."""
    ...
    N = len(frames)
    amusing_indices = set(amusing_indices or [])
    if surprised_indices is None:
        surprised_indices = set(i for i, score in enumerate(scores) if score > 0.2)
    else:
        surprised_indices = set(surprised_indices)
    font_big   = ImageFont.truetype(str(FONT_PATH), 28)
    ...
    for idx, (raw, s, panel) in enumerate(zip(frames, scores, padded_panels)):
        ...
        # Apply yellow highlight overlay for surprised frames
        # if idx in surprised_indices:
        #     # Create a semi-transparent yellow overlay
        #     overlay = Image.new("RGBA", img.size, (255, 255, 153, 80))  # Light yellow with alpha
        ...
        db  = ImageDraw.Draw(bar)
        # spark‑line
        mild_red = (255, 150, 150)  # RGB
        height = db.im.size[1]
        # Create a polygon that goes along the line and down to the bottom
        fill_pts = spark_pts[:idx+1] + [(spark_pts[idx][0], height), (spark_pts[0][0], height)]

        db.polygon(fill_pts, fill=mild_red)
        db.line(spark_pts[: idx + 1], fill=mild_red, width=3)
        x_cur = spark_pts[idx][0]
        db.rectangle([(x_cur - CURSOR_W // 2, 0), (x_cur + CURSOR_W // 2, BAR_H)], fill="black")
        # numeric score
        txt = f"{s:.2f}"
