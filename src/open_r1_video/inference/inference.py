...
from tvsum import load_tvsum_tsv, get_tvsum_video_path, get_tvsum_amusing_frame_indices
random.seed(55)  
np.random.seed(55)
...
def evaluate(
    ...
    if "funqa" in json_path.lower():
        ...
        min_frames = None
    elif "tvsum" in json_path.lower():
        print("Loading TVSum dataset...")
        dataset = load_tvsum_tsv(json_path)
        get_video_path = get_tvsum_video_path
        get_amusing_frame_indices = get_tvsum_amusing_frame_indices
        min_frames = 64
    else:
    ...
    if method_function == "direct" or method_function == "prior_frame_bayesian_approach":
        ...
        from open_r1_video.zero_shot import direct_surprise_tracker
    ...
    for idx, entry in enumerate(tqdm(dataset, desc="Processing entries")):
        ...
        if method_function == "direct":
            ...
            print("Running Qwen Surprise Tracker with use_history...", use_history)
            output = qwen_surprise_tracker(
                frames=frames, window_size=window_size, method=method_function, top_k=topk_hyp, vr=vr, model=model, processor=processor, use_history=use_history
            )
