...
class Qwen2VLGRPOTrainerBelief(Trainer):
    ...
    def __init__(
        ...
        # print("Trainable parameters after applying PEFT: ", sum(p.numel() for p in model.parameters() if p.requires_grad))
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        for example in inputs:
            try:
                ...
                for idx in range(m):
                    ...
                    with torch.no_grad(), autocast():
                        with unwrap_model_for_generation(model, self.accelerator) as unwrapped_model:
                            ...
                            # print(idx, "Surprise scores: ", result["surprise_scores"])
                            top_frames, frame_indices = adaptive_frame_sampling_pdf(
                                scores=result["surprise_scores"], vr=vr, max_frames=len(frames)
                            )

                            prompt_variants = ["Caption this video in 20 words.",
                                            "Describe this video in 20 words.",
                                                "Summarize this video in 20 words.",]
                            ...
                            with torch.no_grad():
                                ...
                                caption_text = caption_text.split("assistant\n")[1].replace(":", "")
                ...
                with autocast(): 
                    ...
                    logps_per_rollout, kl_divergences = score_window_hypotheses(video_frames=frames, window_size=4, all_hypotheses=all_hypotheses, model=model, processor=self.processing_class,
                    memories=all_memories, advantages=adv, ref_model=self.ref_model, beta=beta)
                    ...
                    total_logp_per_rollout = [
                            torch.stack([lp for window_logps in rollout_lps for lp in window_logps]).sum()
                            for rollout_lps in logps_per_rollout
                            if any(len(w) > 0 for w in rollout_lps)
                        ]
                ...
                print("kl", kl_divergences)
                if kl_divergences is not None:
                    mean_kl = kl_divergences.mean().item()
                    self._metrics["kl_divergence"].append(mean_kl)
                    self._metrics["kl_divergence_max"].append(kl_divergences.max().item())
                    self._metrics["kl_divergence_min"].append(kl_divergences.min().item())
        ...
        return total_loss/num_processed if num_processed > 0 else total_loss
